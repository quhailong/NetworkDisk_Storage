var REGIST = {
		...
		doRegist : function() {
			...
			$.ajax({
				url : "http://passport.727pan.cn/getpass/modifyPass",
				type : "post",
				data : user,
				xhrFields : {
	...
	$(function() {
		...
		$("#TANGRAM__PSP_3__verifyCodeSend").click(
				function () {
					if (!$("#TANGRAM__PSP_3__userNameError").text() == "" || $("#inputUsername").val() == "") {
						...
						$.ajax({
							url : "http://passport.727pan.cn/getpass/checkphonesend",
							type : "post",
		...
		$("#regVerfiCode").click(function(){
						...
						$.ajax({
							url : "http://passport.727pan.cn/getpass/forgetphonesend",
							type : "post",
							data : { "username" : username , "VerfyCode" : VerfyCode ,"vcodestr" : vcodestr},
							xhrFields : {
								withCredentials : true
							},
							crossDomain : true,

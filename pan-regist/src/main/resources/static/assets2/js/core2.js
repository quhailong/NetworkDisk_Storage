	...
	function save(ShareId,dest){
		$.ajax({
			url : "http://localhost:9006/api/checkLock",
			type : "post",
			data : {
				"token" : $.cookie('token'),
				"shareId": ShareId,
						...
						$('#Lock').modal({
							closeViaDimmer : 0,
						      onConfirm: function(e) {
						    	  ...
						    	  $.ajax({
						  			url : "http://localhost:9006/api/verifykLock",
						  			type : "post",
						  			data : {
						  				"token" : $.cookie('token'),
						  				"shareId": ShareId,
	...
	function saveShare(ShareId,dest,lockPassword){
			$.ajax({
			url : "http://localhost:9006/api/saveShare",
			type : "post",
			data : {
				"token" : $.cookie('token'),
				"shareId": ShareId,
	...
	function download(ShareId,lockPassword){
				var form=$("<form id='abc'>");//定义一个form表单  
				form.attr("style","display:none");  
				form.attr("method","post");  
				form.attr("action","http://localhost:9007/rest/downloadShare");  
				var input1=$("<input>");  
				input1.attr("type","hidden");
				input1.attr("name","shareId");  
				input1.attr("value",ShareId);
				var input2=$("<input>");  
				...
				input3.attr("name","lockPassword");
				input3.attr("value",lockPassword);

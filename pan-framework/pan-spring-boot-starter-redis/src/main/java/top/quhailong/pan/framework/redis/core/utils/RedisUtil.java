...
public class RedisUtil {
    ...
    public <T> T getAndSet(String key, String value, Class<T> clazz) {
        try {
            String valueJson = redisTemplate.opsForValue().getAndSet(key, value);
            if (StringUtils.isBlank(valueJson)) {
                return null;
            }
            ObjectMapper objectMapper = new ObjectMapper();
            ...
            log.error("redis将给定key的值设为value，并返回key的旧值失败", e);
            return null;
        }
    }
    ...
    public Cursor<Map.Entry<String, String>> hScan(String key, ScanOptions options) {
        return redisTemplate.<String, String>opsForHash().scan(key, options);
    }
    ...
    //    ============================== execute ==============================
    public void executePipelined(RedisCallback<?> action, @Nullable RedisSerializer<?> resultSerializer) {
        redisTemplate.executePipelined(action, resultSerializer);
    }

    public void execute(RedisScript<Object> script, List<String> keys, Object... args) {
